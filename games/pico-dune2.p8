...
81|lAUNCH|99||5|1|1|1|11||||||||1|1||||||||||||||||||0|0|0|0|0|0|1|1||0||||draw_action||action_click]]
...
function m_map_obj_tree(objref, x,y, owner, factory)
  ...
  if (base) new_faction=base[1] newobj.faction,newobj.col1,newobj.col2 = new_faction,base[2],base[3]
  ...
  if objref.type==2 then
    ...
    if newobj.id==6 and newobj.parent==nil then
     ...
     m_map_obj_tree(obj_data[32],ux,uy,nil,newobj)
    end
  else
  -- unit props  
  ...
  for unit in all(units) do
    if unit then
      --if unit.cor and costatus(unit.cor) != "dead" then
        if(unit.cor)assert(coresume(unit.cor, unit))
      --else
      --  unit.cor = nil
      --end
...

