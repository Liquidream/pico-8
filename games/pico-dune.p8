...
function _init()
 ...
 cursor = {
  ...
  get_hitbox=function(self)
   return {
    x=self.x+(not ui_collision_mode and camx or 0),
    y=self.y+(not ui_collision_mode and camy or 0),
    w=1,
    h=1
   }
  end,
  ...
  if selected_obj.owner==1 then   
   ...
   if selected_obj.life<selected_obj.hitpoint   
    ...
    and (selected_obj.type==2
     ...
     repair_obj=m_obj_from_ref(obj_data[80], 117,29, 5, {}, nil,draw_action, repair_click) 
     repair_obj:draw()
   end
   -- fire palace weapon?   
   if (selected_obj.id==19 
    ...
    then
     launch_obj=m_obj_from_ref(obj_data[81], 109,31, 5, {}, nil,draw_action, launch_click) 
     launch_obj:draw()  
   end
