...
cx=function(self)
return{
cy=self.cy+(not cz and da or 0)+2,
db=self.db+(not cz and dc or 0)+1,
w=1,
h=1
}
end,
...
self.cy,self.db,self.w/8,self.h/8)
end
}
for cs=1,2 do
for ch=0,31 do
...
cm(de,dk*8,dl*8)
if(de.type==1 and de.speed>0) mset(cg,ch,0)
end
end
end
end
...
if(self.cl<self.hitpoint*.33 and self.altframe==nil and rnd"10"<1) fv(self.cy+3.5,self.db+3.5,1,.1,-.02,.05,-.002,80,bu("10,8,9,6,5",","),rnd"2"<1 and 0xa5a5.8 or 0)
self.fo=0
end,
fw=function(self)
local cl=self.cl
...
for gq,gr in pairs(fc) do
if fe[gq]==nil and gr!=""then
fe[gq]=gr
end
end
...
function gn(gs,fe)
if(gt(j[fe.owner])+gs<0) return false
j[fe.owner]+=sgn(gs)*shr(abs(gs),16)
if(fe.owner==1) sfx"63"
return true
end
function gf(cy,db,gu)
fv(cy,db,2,
0,0,.1,0,gu==1 and 5 or 30,bu("5,7,10,8,9,2",","),rnd"2"<1 and 0xa5a5.8 or 0)
end
function ey(gv)
if(gv.owner!=1 and gv.en!=4) return
local gw=gv.type==2 and 3 or 2
for ed=-gw,gw do
for ee=-gw,gw do
local gx,gy=flr(gv.cy/8)+ed,flr(gv.db/8)+ee
bg[gx][gy]=16
gz(gx,gy)
for eu=-1,1 do
for et=-1,1 do
...
local hc=mget(cg,ch)
local hd=sget((hc*8)%128+4,(hc*8)/16)
if(bg[cs/2][cw/2]==16) o[(cs/2/2)..","..(cw/2/2)]=hd!=11 and hd or 15
end
end
end
...
local gx,gy=hi:ci()
if hi.owner==1 or(bb and bg[gx][gy]==16) then
...
local ix=hj:gp()
p[ix],self.fa=(p[ix] or 1000)-1,(self.fa or 0)+.5
if p[ix]<=0 then
local ea,eb=self:ci()
for ee=-1,1 do
for ed=-1,1 do
...
while self.fa>0 do
self.fa-=1
if(flr(self.fa)%4==0 and tonum(hm)<hf) gn(2,self)
yield()
end
...
local jk=hj.z>1
if jj then
local jl=r and r[hj.du][31] or false
...
and(bm.type==2
or bm.speed==0) then
kx=dq(cn[80],117,28,5,{},nil,by,repair_click)
kx:dd()
end
if(bm.id==19
and bm.fn<=0)
or bm.id==35
then
ky=dq(cn[81],109,29,5,{},nil,by,cd)
ky:dd()
end
end
end
...

__map__
...

...

...

