pico-8 cartridge // http://www.pico-8.com
version 32
__lua__
-- rotate perf test
function _init()

 -- which method to test
 method=2

 sprlist={38,40,50,51,52,53,54,55,56,57,71,72,73,87}
 z=1
 r=0
 flash=0
end

function _update60()
 if (btn(➡️)) r+=0.01
 if (btn(⬅️)) r-=0.01
 if (btn(⬆️)) z=min(z+1,30)
 if (btn(⬇️)) z=max(z-1,1)
 if (btn(❎)) flash=10
 
 flash=max(flash-0.4,1)
end

function _draw()
 cls(15)
 local x=20 y=50
 
 -- method 1 ------------------
 -- my orig (slower) code 
 -- calls rotspr twice
 -- (first pass for shadow)
 -- ---------------------------
if method==1 then

 for sprnum in all(sprlist) do
	 for i=1,2 do
 	 rspr(sprnum%16*8,sprnum\16*8,
 		 x,y-(i==2 and z or 0),
 			.25-r, 1, 11, i==1 and 5 or nil)
	 end
	 x+=12
	 if(x>100) x=20 y+=16
	end
	
elseif method==2 then

 -- method 2 ------------------
 -- draw twice *within* rotspr
 -- ---------------------------
 for sprnum in all(sprlist) do	 
	 rsspr(sprnum%16*8,sprnum\16*8,
		 x,y,.25-r, 1, 11, z, flr(flash)%2==0 and 7 or nil)
	 x+=12
	 if(x>100) x=20 y+=16
	end

else

 -- method 3 ------------------
 -- same as #2, but use caching
 -- to avoid re-drawing
 -- ---------------------------
 for sprnum in all(sprlist) do	 
  pset_cache={}
	 rssprc(sprnum%16*8,sprnum\16*8,
		 x,y,.25-r, 1, 11, z, flr(flash)%2==0 and 7 or nil)
	 x+=12
	 if(x>100) x=20 y+=16
	end
	
 
end
	
 rectfill(0,0,127,8,1)
 print("method "..method.." = "..stat(1).."%",2,2,7)
end
-->8
-- my first modified rotspr code 
-- (to allow for trans cols)
-- orig by freds72
-- https://www.lexaloffle.com/bbs/?pid=52525#p52541
function rspr(sx,sy,x,y,a,w,trans,single_col)
	local ca,sa=cos(a),sin(a)	
	local ddx0,ddy0,mask = ca,sa,0xfff8<<w-1
	w*=4
	ca*=w-0.5
	sa*=w-0.5
	local dx0,dy0,w = sa-ca+w,-ca-sa+w,2*w-1	
	for ix=0,w do
		local srcx,srcy=dx0,dy0
		for iy=0,w do
			if ((srcx|srcy) & mask)==0 then
				local c=sget(sx+srcx,sy+srcy)
				if (c!=trans) pset(x+ix,y+iy, single_col or c)
			end
			srcx-=ddy0
			srcy+=ddx0
		end
		dx0+=ddx0
		dy0+=ddy0
	end
end
-->8
-- shadow rotspr 
-- (modded to do shadow inside)
function rsspr(sx,sy,x,y,a,w,trans,
  shad_dist,single_col)
	local ca,sa=cos(a),sin(a)	
	local ddx0,ddy0,mask = ca,sa,0xfff8<<w-1
	w*=4
	ca*=w-0.5
	sa*=w-0.5
	local dx0,dy0,w = sa-ca+w,-ca-sa+w,2*w-1	
	for ix=0,w do
		local srcx,srcy=dx0,dy0
		for iy=0,w do
			if ((srcx|srcy) & mask)==0 then
				local c=sget(sx+srcx,sy+srcy)
				if c!=trans then
				 if shad_dist then
				  pset(x+ix,y+iy, 5)
				 end
				 pset(x+ix,y+iy-z, single_col or c)
				end				 
			end
			srcx-=ddy0
			srcy+=ddx0			
		end
		dx0+=ddx0
		dy0+=ddy0
	end
end
-->8
-- shadow rotspr (w/cache)
-- !!aborted-cant work this way
--  (as shadow always in cache)
local n=0
function rssprc(sx,sy,x,y,a,w,trans,
  shad_dist,single_col)
	local ca,sa=cos(a),sin(a)	
	local ddx0,ddy0,mask = ca,sa,0xfff8<<w-1
	w*=4
	ca*=w-0.5
	sa*=w-0.5
	local dx0,dy0,w = sa-ca+w,-ca-sa+w,2*w-1	

	for ix=0,w do
		local srcx,srcy=dx0,dy0
		for iy=0,w do
			if ((srcx|srcy) & mask)==0 then
				local c=sget(sx+srcx,sy+srcy)
				if c!=trans then				 
				 psetc(x+ix,y+iy-z, single_col or c)
				 if shad_dist then
				  psetc(x+ix,y+iy, 5)
				 end
				end 
			end
			srcx-=ddy0
			srcy+=ddx0			
			
			n+=1
  	if(n>20)stop()
		end
		dx0+=ddx0
		dy0+=ddy0
	end
end

-- cached pset which skips 
-- redraw within a draw period
pset_cache={}
function psetc(x,y,col)
 local k=x..","..y
	if not pset_cache[k] then
	 pset(x,y,col)
	 pset_cache[k]=true
	 ?(k.."="..col)
	end	
end
__gfx__

__label__


