  ...
  get_hitbox=function(self)
   ...
   update=function(self)
     ...
     if self.hit>0 and self.created_by>0 and id!=42 then        
       ...
       -- req repair pickup
       if (self.is_repairable and self.moves and life<99 and self.state<=6) return_to_fact(self,safe_rnd_has_obj(self.created_by,9) or self.last_fact)
       -- lose soldiers
     ...
     if self.process>0 
      ...
      elseif self.process==2 and life>self.hitpoint then
        ...
        if self.procstep>=8 then
         -- build/repair (only if have money)
         -- note: only reset the procstep if have funds (no free build/upgrades!)
         if (transact(-4,self.process==1 and self.parent or self)) self.procstep=0 self.spent+=4
        else
         -- just continue
         self.procstep+=1
      ...
      if self.capacity and last_fact.id==17 then 
       ...
       while self.capacity>0 do
        ...
        if flr(self.capacity)%8==0 then
         total_spice[self.owner]+=.5 --1/4 scale  (700 credits for full unload in Dune2JS)
...
function do_attack(unit, target, manual_attack)
 ...
 if id!=19 then 
  ...
  -- palace weapons
  do_attack(m_map_obj_tree(obj_data[({24,25,50,50.5})[unit.faction]], unit.x,unit.y, unit.owner, unit), target)      
...

